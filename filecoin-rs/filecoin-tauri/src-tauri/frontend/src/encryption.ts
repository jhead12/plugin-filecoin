...
const ENCRYPTION_KEY = (() => {
    ...
    if (key.length !== KEY_LENGTH) {
        throw new EncryptionError(
            `ENCRYPTION_KEY must be ${KEY_LENGTH} bytes (got ${key.length} bytes)`,
        );
    }
    return key;
})();
...
export function isValidKey(key: Buffer): boolean {
    ...
    // (No need to re-export constants defined in the same file)
    // export { ENCRYPTION_KEY, ALGORITHM, KEY_LENGTH, IV_LENGTH };
}
