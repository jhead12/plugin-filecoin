...
export class AgentIntentHandler {
    ...
    async handleIntent(intent: Intent): Promise<string> {
        ...
        try {
            ...
            return await invoke<string>("execute_agent_intent", {
                action,
                data,
                agent,
            });
        } catch (error) {
            this.logger.error("Intent handling failed:", {
                action,
                agentId,
                error,
            });
            throw new AgentError(
                `Failed to handle intent: ${error}`,
                "INTENT_ERROR",
                { error },
            );
        }
...
export class FilecoinDatabaseAdapter
    extends DatabaseAdapter<any>
