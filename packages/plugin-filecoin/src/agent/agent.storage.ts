...
const defaultFilecoinClient: FilecoinClient = {
  upload: async (data: string): Promise<string> => {
...
export class AgentStorage {
  ...
  async upload(data: string): Promise<string> {
    return this.withRetry(async () => {
      const encryptedData = Buffer.from(encryptData(data, this.config.encryptionAlgorithm)).toString('hex');
  ...
  async storeAudioAsset(asset: Omit<AudioAsset, 'id' | 'createdAt'>): Promise<AudioAsset> {
  ...
  async getAudioAsset(storageId: string): Promise<AudioAsset> {
  ...
  async storeCommMessage(message: Omit<CommMessage, 'timestamp'>): Promise<string> {
