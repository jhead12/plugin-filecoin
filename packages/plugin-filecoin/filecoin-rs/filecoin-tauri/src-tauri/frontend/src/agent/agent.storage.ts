...
export class AgentStorage {
    ...
    private async withRetry<T>(
        ...
        for (
            ...
            try {
                ...
                lastError = error;
                this.logger.warn(
                    `${operationName} failed (attempt ${attempt}/${this.config.maxRetryAttempts}): ${error.message}`,
                );
        ...
        throw new Error(
            `${operationName} failed after ${this.config.maxRetryAttempts} attempts: ${lastError?.message}`,
        );
    }
    ...
    async upload(data: string, walletAddress: string): Promise<string> {
        return this.withRetry(async () => {
            ...
            const storageId = await this.client.upload(
                encryptedData,
                walletAddress,
            );
    ...
    async storeAudioAsset(
        asset: Omit<AudioAssetDTO, "id" | "createdAt">,
        ...
        this.logger.info("Audio asset stored", {
            name: asset.name,
            storageId,
            walletAddress,
        });
    ...
    async getAudioAsset(
        ...
        if (errors.length > 0) {
            throw new Error(
                `Invalid audio asset data: ${errors.map((e) => e.toString()).join(", ")}`,
            );
        }
        return asset;
    }

    async storeCommMessage(
        message: Omit<CommMessageDTO, "id" | "timestamp">,
        walletAddress: string,
        ...
        this.logger.info("Communication message stored", {
    ...
    async storeDocument(
        doc: Omit<DocumentDTO, "id" | "createdAt">,
        walletAddress: string,
    ): Promise<DocumentDTO> {
        ...
        if (
            document.type === "ics" &&
            !document.data.includes("BEGIN:VCALENDAR")
        ) {
            throw new Error("Invalid iCalendar format");
        }
        if (
            document.type === "excel" &&
            !document.data.startsWith("data:application/vnd.openxmlformats")
        ) {
    ...
    async getDocument(
        ...
        if (errors.length > 0) {
            throw new Error(
                `Invalid document data: ${errors.map((e) => e.toString()).join(", ")}`,
            );
        }
        return doc;
    }
