...
const sequelize = new Sequelize({
    ...
    storage:
        dialect === "sqlite"
            ? process.env.DB_STORAGE || "performance.db"
            : undefined,
    host:
        dialect === "postgres"
    ...
    port:
        dialect === "postgres"
            ? Number(process.env.PM_DB_PORT) || 5432
            : undefined,
    username:
        dialect === "postgres"
            ? process.env.PM_DB_USER || "performance_user"
            : undefined,
    password:
        dialect === "postgres"
            ? process.env.PM_DB_PASSWORD || "password123"
            : undefined,
    database:
        dialect === "postgres"
            ? process.env.PM_DB_NAME || "performance_db"
            : undefined,
