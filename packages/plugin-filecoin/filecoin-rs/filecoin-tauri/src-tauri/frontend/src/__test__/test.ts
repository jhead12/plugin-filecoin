...
const STORACHA_API_ENDPOINT =
    process.env.STORACHA_API_ENDPOINT || "https://api.storacha.network";
...
export async function uploadToStoracha(data: string): Promise<string> {
    if (!STORACHA_API_TOKEN || !STORACHA_API_ENDPOINT) {
        throw new Error(
            "Storacha API token or endpoint not configured in environment variables",
        );
    }

    try {
        const encryptedData = encrypt(data); // Returns Uint8Array or string, adjust based on encryption.ts
        const response = await storachaClient.post("/data", {
            data: encryptedData,
        });
        ...
        console.error(
            "Error uploading data to Storacha:",
...
export async function downloadFromStoracha(cid: string): Promise<Uint8Array> {
    ...
    try {
        ...
        console.log(
            `Data downloaded from Storacha (ID: ${cid}):`,
            decryptedData,
        );
        ...
        const err = error as AxiosError;
        console.error(
            "Error downloading data from Storacha:",
            err.response?.data || err.message,
        );
        throw error;
    }
