...
} from "./filecoin-rs-bindings";
import fs from "fs/promises";
import path from "path";
import { encrypt, decrypt } from "./encryption";
import { checkFilecoinConnection, connectToFilecoin } from "./utils";
...
async function main(): Promise<void> {
    ...
    const backupPath = path.join(
        backupDir,
        process.env.BACKUP_SUBPATH || "backup.bin",
    );
    const destinationPath = path.resolve(
        __dirname,
        process.env.DESTINATION_PATH || "../public",
    );
    ...
    if (storageDataId) {
        try {
            ...
            logger.info(
                "Data downloaded (hex):",
                Buffer.from(downloadedData).toString("hex"),
            );
            logger.info(
                "Decrypted data matches original?",
                decryptedString === dataToBackup,
            );
        } catch (error) {
    ...
    try {
        ...
        logger.info(
            "Memory created in FilecoinDatabaseAdapter with runtime UUIDs",
        );
    } catch (error) {
    ...
    try {
        ...
        logger.info(
            `Restored from ${backupPath} to ${destinationPath}: ${restoreSuccess}`,
        );
    } catch (error) {
